...
def ga_runner(env, seed: int):
    return run_ga(env=env, fitness_fn=evaluate, cfg=GAConfig(), seed=seed)

def aco_runner(env, seed: int):
    return run_aco(env=env, fitness_fn=evaluate, cfg=ACOConfig(), seed=seed)
def de_ga_runner(env, seed: int):
    cfg = GAConfig(
        population_size=60,
        crossover_rate=0.9,
        mutation_rate=0.05,
    ...
    cfg.mutation_rate = 0.2  # أقوى (DE-like behavior)
...
def main() -> None:
    ...
    parser.add_argument(
        "--configs", nargs="+",
                     choices=["ga", "aco", "hybrid", "hybrid_island", "hybrid_sharing", "de_ga"],
                     default=["ga", "aco", "hybrid", "hybrid_island", "hybrid_sharing", "de_ga"],
    )
